...
test("T34: max output tokens are capped by model spec", () => {
  ...
  assert.equal(capMaxOutputTokens("claude-opus-4-6", 200000), 128000);
  assert.equal(capMaxOutputTokens("claude-sonnet-4-6", 200000), 64000);
...
test("T38: modelSpecs exposes centralized helpers with alias and prefix lookup", () => {
  ...
  assert.equal(getModelSpec("claude-opus-4.6").maxOutputTokens, 128000);
  assert.equal(getModelSpec("claude-sonnet-4-6").contextWindow, 1000000);
  ...
  assert.equal(resolveModelAlias("claude-sonnet-4.6"), "claude-sonnet-4-6");
